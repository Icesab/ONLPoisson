...
2. 不换语言，不把 MATLAB/C 整体改写成 Python。
3. 不做大重构，不做“顺手优化”。
...
原因：它实现的正是论文里的 NMISE 思路：去掉真值为 0 的像素后，计算
\((\hat u-u)^2/u\) 的平均。这个文件可以作为最终统计指标继续使用。

### 1.2 `nlm_poisson004.c`
**保留整体结构**，只做很小的论文对齐修改。
...
不要重写成一个新算法；只在公式不一致处改 2~3 个点。

### 1.3 `demo_onl.m`
**不要沿用它的实验逻辑**，但可以留作“旧调用方式参考”。

它的问题不是“不能跑”，而是“和论文 2021 版实验不对齐”。

### 1.4 `nlm_poisson004.mexw64` / `nlm_poisson004.dll`
保留文件，但**不要把它们当作复现实验的依赖**。

它们只是旧的 Windows 编译产物。真正复现时应当从 `nlm_poisson004.c` 重新编译 MEX。
